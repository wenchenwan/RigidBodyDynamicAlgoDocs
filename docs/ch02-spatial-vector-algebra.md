...
```
空间加速度线分量 = [0, 0, 0]           ← 恒为零
P 点经典加速度    = [-2.205, 0, 0]      ← 正是向心加速度 -ω²r = -2.1²×0.5
```
...
```
Cholesky 的 g₁ = [ 1.141 -0.520  0.932 ...]
特征分解的 g₁ = [-0.295  0.164 -0.061 ...]      ← 完全不同，但都对
```
...
$$
f=m\,\ddot c\tag{2.70 牛顿}
\qquad
n_C=\bar I_C\dot\omega+\omega\times\bar I_C\omega\tag{2.71 欧拉}
$$
