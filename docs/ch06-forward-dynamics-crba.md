...
I^c_i=I_i+I^c_{i+1}
\qquad\qquad
H_{ij}=S_i^{\mathsf T}I^{c}_{\max(i,j)}S_j\tag{6.15, 6.16}
$$
...
for i = N_B to 1 do
    if lam(i) != 0 then
        I^c_lam(i) = I^c_lam(i) + X[lam(i),i]^* I^c_i * X[i,lam(i)]
    end
...
for k = n to 1 do                           for k = n to 1 do
    ...
    i = lam(k)                                  while i != 0 do
    while i != 0 do                                 a = H_ki / H_kk
        H_ki = H_ki / H_kk                          j = i
        i = lam(i)                                  while j != 0 do
    end                                                 H_ij = H_ij - a*H_kj
    i = lam(k)                                          j = lam(j)
    while i != 0 do                                 end
        j = i                                       H_ki = a
        while j != 0 do                             i = lam(i)
...
```
for i = 1 to n do
    lam'(i) = i - 1          <- 巧妙的初始化：一次正确设好 n - N_B 个元素
end
map(0) = 0
for i = 1 to N_B do
    map(i) = map(i - 1) + n_i
...
for i = 1 to n do                 for i = n to 1 do
    ...
    while j != 0 do                   j = lam(i)
        x_i = x_i - L_ij*x_j          while j != 0 do
        j = lam(j)                        x_j = x_j - L_ij*x_i
    end                                   j = lam(j)
    x_i = x_i / L_ii                  end
end                               end
```
...
$$
D_1=\sum_{k=1}^{n}(d_k-1)
\qquad\qquad
D_2=\sum_{k=1}^{n}\frac{d_k(d_k-1)}{2}\tag{6.26, 6.27}
$$
